...
from utils.config_handler import db_conf, agent_conf
...
# Scenario Agent模块
async def scenario_agent(state: ScenarioState) -> ScenarioState:
    """基于预处理的输入生成想定"""
    logger.info("开始生成想定")
    # 初始化LightRAG
    rag = RAG()
    ...
    async for chunk in react_agent.aexecute_stream(state["user_input"], load_text_examples, state["session_id"]):
        scenario_text = scenario_text + chunk

    # scenario_str = react_agent.execute_stream(state["user_input"], load_json_examples, state["session_id"])
    # scenario = {}
    # try:
...
def output_scenario(state: ScenarioState) -> ScenarioState:
    ...
    state["final_scenario"] = state["scenario_text"]
    logger.info(f"最终想定输出完成！")
    return state
...
scenario_graph.add_edge("scenario_agent", "machine_review")
scenario_graph.add_edge("machine_review", "output_scenario")
...
# scenario_graph.add_edge("reflection_iteration", "scenario_agent")
...
async def run_scenario_agent(user_input: str):
    ...
    async for node, chunk in graph.astream(initial_state, config=config, stream_mode="messages", subgraphs=True):
        if isinstance(chunk[0], AIMessageChunk):
            if chunk[0].content:
                # yield {"content": chunk[0].content}
                yield chunk[0].content.strip()
